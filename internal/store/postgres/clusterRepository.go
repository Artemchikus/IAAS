...
func (r *ClusterRepository) Create(ctx context.Context, cluster *models.Cluster) error {
	...
	query := `
	INSERT INTO cluster 
	(location, url, admin_name, admin_email, admin_password, admin_project_id, admin_domain_id)  
	...
	row, err := r.store.db.Query(
		...
		cluster.Admin.ProjectID,
		cluster.Admin.DomainID)
	if err != nil {
		return err
	}
	defer row.Close()
...
func scanIntoCluster(rows *sql.Rows) (*models.Cluster, error) {
	...
	if err := rows.Scan(
		...
		&cluster.Admin.Password,
		&cluster.Admin.ProjectID,
		&cluster.Admin.DomainID); err != nil {
		return nil, err
	}
...
func (r *ClusterRepository) createClusterTable(ctx context.Context) error {
	...
	query := `CREATE TABLE IF NOT EXISTS cluster (
		...
		admin_project_id VARCHAR(50) NOT NULL UNIQUE,
		admin_domain_id VARCHAR(50) NOT NULL
	)`
